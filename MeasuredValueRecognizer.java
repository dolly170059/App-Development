...
public class MeasuredValueRecognizer {
	...
	public static String checkMeasureValue(String input) {
		...
		while (running) {
			...
			case 6: 
                // State 6 has one valid transition. It is addressed by an if statement.
			...
			case 7: 
                // State 7 has one valid transition.  It is addressed by an if statement.
				
				// This is a final state
				finalState = true;
				
				// If the current character is in the range from 1 to 9, it is accepted and we stay in this
				// state
				if (currentChar >= '0' && currentChar <= '9') {
					nextState = 7;
					break;
